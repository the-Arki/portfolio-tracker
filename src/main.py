...
class Portfolios:
    ...
    def __init__(self, currency="HUF"):
        ...
        try:
            self.portfolio_names = io_manager.read_json("files/portfolio_names.json")
            for name in self.portfolio_names.keys():
                self.instances[name] = Portfolio(
                    tr_dict=self.all_transactions[name],
                    name=name,
                    currency=self.portfolio_names[name],
                )
        except (json.decoder.JSONDecodeError, FileNotFoundError):
    ...
    def create_instance(self, name, currency):
        ...
        self.instances[name] = Portfolio(
            tr_dict=self.all_transactions[name], name=name, currency=currency
        )
    ...
    def buy_equity(self, name, date, ticker, amount, unit_price, fee, currency):
        ...
        if self.instances[name].cash._validate_transaction(tr):
            transaction = self.instances[name].stock.buy(
